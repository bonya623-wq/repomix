...
_INSTANT_MAP = {
    ...
    "[error]":                     ("⚠️", None),
    "подходящих лотов не найдено": ("😶", "Подходящих лотов нет"),  # FIX: перенесено сюда для срабатывания переключения игры
}
...
# БАГ #5 ИСПРАВЛЕН: добавлены паттерны для price_check логов
_PRICE_MAP = {
    "запускаем проверку цен":      ("💰", "Запуск проверки цен..."),
    "price_check":                 ("💰", None),
    "проверка цен завершена":      ("✅", "Проверка цен завершена!"),
    "run_price_check":             ("💰", None),
    "проверка цен |":              ("💰", None),
    "лотов | порог":               ("💰", None),
    "цена с листинга":             ("💰", None),
    "цена со страницы":            ("💰", None),
    "цена не найдена":             ("⚠️", None),
    "лот продан - пропускаем":     ("🛒", None),
    "нужно обновить:":             ("✏️", None),
    "цена успешно обновлена":      ("✅", None),
    "fp:":                         ("📊", None),
    "ok изменение <":              ("✅", None),
    "миграция:":                   ("🔄", None),
    "готово | обновлено":          ("✅", None),
    "шаг 1:":                      ("🔍", None),
    "шаг 2:":                      ("🔧", None),
}
    ...
    try:
        ...
        for raw in proc.stdout:
            ...
            if is_price_check:
                ...
                # Каждый лот: "[N/M] ..."
                if re.search(r"\[\d+/\d+\]", line_lower) and "/25" not in line_lower[:3]:
                    pc_checked += 1
